...
import AddAllergieModal from "./AddAllergieModal";
...
const ClientProfile = ({
  ...
  return (
    <>
      <div className="w-full bg-white mx-auto overflow-hidden rounded-xl">
        ...
        <div className="px-6 sm:px-10 py-6 sm:py-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:items-center md:justify-between">
            ...
            {/* Right action — opens modal */}
            <div className="flex justify-center md:justify-end mt-4 md:mt-0">
              <button
                onClick={() => setShowModal(true)}
                className="rounded-lg bg-[#635BFF] cursor-pointer font-manrope px-4 py-2.5 text-sm font-medium text-white hover:opacity-90 active:scale-[0.97] transition-all"
              >
                Book Now
              </button>
            </div>
      ...
      {/* Modal */}
      {showModal && <AddAllergieModal onClose={() => setShowModal(false)} />}
    </>
  );
};
